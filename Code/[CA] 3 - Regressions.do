...
//Load data
use "Data/workingConfirmatoryData.dta"
...
outreg2  using "Tables/tableA7",  tex stats(coef se)  replace
...
outreg2  using "Tables/tableA7",  tex stats(coef se)  append
...
outreg2  using "Tables/tableA8",  tex stats(coef se)  replace
...
outreg2  using "Tables/tableA8",  tex stats(coef se) append
...
twoway  (rcap ub lb interactionValue, lcolor(navy)) (scatter coef interactionValue, color(navy))  if effectVariable=="Awareness",/*
	...
	*/ legend(off) /*
	*/ yline(0, lcol(black))
graph export "Graphs/figureA3.eps", as(eps) preview(off) replace	
